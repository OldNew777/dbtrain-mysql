...
class FixedRecord : public Record {
 public:
   ...
   */
  virtual Size Load(const uint8_t* src) override;
  /**
   * @brief 记录序列化
   *
   * @param dst 序列化结果存储位置
   ...
   */
  virtual Size Store(uint8_t* dst) const override;
  /**
   * @brief 从String数据构建记录
   *
  ...
  virtual void Sub(const std::vector<Size>& iPos) override;
  virtual void Add(Record* pRecord) override;
  virtual void Remove(FieldID nPos) override;
